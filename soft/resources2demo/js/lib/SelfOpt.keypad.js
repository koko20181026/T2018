...
function moveCursorSet(targetId, cursorIndex){
	setKeyCursor(targetId, cursorIndex);
}
...
function keyCtr(){
  ...
  // 数字桁数10まで99億(桁数数字１０桁まで入力可、コンマまで１３桁)
  ...
  if(amt.length > 6 || manCnt>0 || senFlg){
	  $(".keypad-man").addClass("itemDisabled");// 万　非活性(半透明、クリック出来ない)
  }

  if(amt.length > 1 && !senFlg  || senFlg ){
	  $(".keypad-sen").addClass("itemDisabled");// 千　非活性(半透明、クリック出来ない)
  }
}
...
function setOldTargetId(target){
	oldTargetId = target;
...
//カーソル位置設定
function setKeyCursor(targetId, p_cursorIndex){
    $("#"+ targetId).get(0).setSelectionRange(p_cursorIndex, p_cursorIndex);
}
...
function dakutenKeyControl(){
    ...
    if(lbm_Util_ItemIsNull(layoutName) == false){
        if(layoutName == kanjiLayout){
        	//濁点
            markEnableKeyDakuten = ["か","き","く","け","こ","た","ち","つ","て","と","は","ひ","ふ","へ","ほ","う"];
            markEnableKeyHandakuten = ["は","ひ","ふ","へ","ほ"];
        } else if(layoutName == kanaAddrLayout || layoutName == kanaNameLayout){
        	//濁点
        	markEnableKeyDakuten = ["カ","キ","ク","ケ","コ","タ","チ","ツ","テ","ト","ハ","ヒ","フ","ヘ","ホ","ウ"];
